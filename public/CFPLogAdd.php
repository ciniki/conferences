...
// Description
// -----------
// This method will add a new cfp log for the tenant.
//
// Arguments
// ---------
// api_key:
// auth_token:
// tnid:        The ID of the tenant to add the CFP Log to.
//
// Returns
// -------
// <rsp stat="ok" id="42">
...
function ciniki_conferences_CFPLogAdd(&$ciniki) {
    ...
    $rc = ciniki_conferences_checkAccess($ciniki, $args['tnid'], 'ciniki.conferences.CFPLogAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.conferences.cfplog', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    if( isset($args['categories']) ) {
        ...
        $rc = ciniki_core_tagsUpdate($ciniki, 'ciniki.conferences', 'cfplogtag', $args['tnid'],
