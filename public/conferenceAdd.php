...
// Description
// -----------
// This method will add a new conference for the tenant.
//
// Arguments
// ---------
// api_key:
// auth_token:
// tnid:        The ID of the tenant to add the Conference to.
//
// Returns
// -------
// <rsp stat="ok" id="42">
...
function ciniki_conferences_conferenceAdd(&$ciniki) {
    ...
    $rc = ciniki_conferences_checkAccess($ciniki, $args['tnid'], 'ciniki.conferences.conferenceAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.conferences.13', 'msg'=>'You already have a conference with this name, please choose another name.'));
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.conferences.conference', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
