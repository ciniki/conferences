...
// <rsp stat='ok' />
//
function ciniki_conferences_attendeeUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_conferences_checkAccess($ciniki, $args['business_id'], 'ciniki.conferences.attendeeUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.conferences.attendee', $args['attendee_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['business_id'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.conferences.attendee', 'object_id'=>$args['attendee_id']));
