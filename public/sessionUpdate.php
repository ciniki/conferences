...
// <rsp stat='ok' />
//
function ciniki_conferences_sessionUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_conferences_checkAccess($ciniki, $args['business_id'], 'ciniki.conferences.sessionUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    //
    // Update the Conference Session in the database
    //
    ciniki_core_loadMethod($ciniki, 'ciniki', 'core', 'private', 'objectUpdate');
    $rc = ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.conferences.session', $args['session_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['business_id'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.conferences.session', 'object_id'=>$args['session_id']));
