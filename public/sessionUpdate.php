...
function ciniki_conferences_sessionUpdate(&$ciniki) {
    ...
    $rc = ciniki_conferences_checkAccess($ciniki, $args['tnid'], 'ciniki.conferences.sessionUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.conferences.session', $args['session_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.conferences.session', 'object_id'=>$args['session_id']));
